...
export default function CarLoanPage() {
  ...
  // Fetch car loans from API
  useEffect(() => {
    const fetchLoans = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${API_URL}/api/car-loans/all`);
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-slate-50 p-8 flex items-center justify-center">
          <p className="text-slate-400">Loading car loans...</p>
        </div>
      </>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h1 className="text-4xl font-bold text-slate-800">Car Loans</h1>
          </div>
          <p className="text-slate-600 text-lg">Get behind the wheel with our competitive car financing options</p>
        </div>
        ...
        <div className="mb-8">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-12 pr-6 py-3 border border-slate-200 rounded-lg focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100"
            />
          </div>
        </div>
...
const LoanCard = ({ loan }) => {
  return (
    <Link to={`/productportfolio/carloan/apply/${loan.id}`}>
      <div className="bg-white rounded-2xl shadow-md hover:shadow-xl transition-all p-6 cursor-pointer border border-slate-100 hover:border-orange-300">
        ...
        {/* Apply Button */}
        <button className="w-full bg-orange-600 text-white font-semibold py-2 rounded-lg hover:bg-orange-700 transition-colors flex items-center justify-center gap-2">
