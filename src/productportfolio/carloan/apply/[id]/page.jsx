...
const ApplyCarLoanPage = () => {
  ...
  useEffect(() => {
    const fetchLoanData = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${API_URL}/api/car-loans/${loanId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/car-loan-applications/apply`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        setFormData({ fullName: '', mobileNumber: '', email: '', loanAmount: '', carType: 'New', agreed: false });
      } else {
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-10">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Back Button */}
        <Link to="/productportfolio/carloan" className="flex items-center gap-2 text-orange-600 font-semibold mb-6 hover:gap-3 transition-all">
          <ArrowLeft size={18} /> Back to Car Loans
        </Link>
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col md:flex-row">
                  ...
                  <ul className="space-y-2">
                    {loan.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start gap-2">
                        <CheckCircle2 className="text-orange-600 mt-0.5 shrink-0" size={16} />
          ...
          <div className="w-full md:w-3/5 p-8 md:p-12">
            <h3 className="text-2xl font-bold text-slate-800 mb-2">Apply for Car Loan</h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 border border-slate-200 rounded-lg focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100"
                    required
                  />
                </div>
              </div>
              ...
              {/* Car Type */}
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Car Type *</label>
                <div className="relative">
                  <Car className="absolute left-4 top-3 text-slate-400" size={18} />
                  <select
                    name="carType"
...
export default ApplyCarLoanPage;
